...
## Load Sector Dataset ##
sectors <- read_csv("https://raw.githubusercontent.com/darshils2001/info201-project/main/datasets/National_Sector_Dataset.csv")
...
my_server <- function(input, output) {
  ...
  output$sector_boxplot <- renderPlotly({
    ...
    # Update name and data type
    selected_sectors$ESTIMATE_PERCENTAGE <- as.numeric(
      gsub("[\\%,]", "", selected_sectors$ESTIMATE_PERCENTAGE)
    )

    group_sectors <- selected_sectors %>%
      group_by(Sector) %>%
      rename(Percent = ESTIMATE_PERCENTAGE)

    p <- ggplot(data = group_sectors) +
      geom_boxplot(
        mapping = aes(x = Sector, y = Percent),
        fill = "#CB4335"
      ) +
      ...
      labs(
        title = "Percentage Revenue Changes - Week of October 04, 2020",
        x = "Sectors", y = "Percent (%)"
      )
  ...
  output$boxplot_text <- renderUI({
    boxplot_intro <- "This boxplot was generated to further explore the
    variance within each job sector as well as across each sector as a result
    of COVID-19 in October. User can select sector(s) by typing the sector(s)
    they want to see the statistics for."

    boxplot_description <- "From the plot, we can see how the effect varies
    across the board, especially when comparing with the median of that sector.
    ...
    high percentage of change for each business in the sector.Most of other
    sectors seems to recover from the pandamic, with the median of
    change falling below 13 percent."

    HTML(paste(boxplot_intro, boxplot_description, sep = "<br/><br/>"))
  })
